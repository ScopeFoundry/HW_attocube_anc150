...
namespace WinspecInterfaceService
{
    // Structure that defines the header when sending the data over.
    ...
    // Simple class for a logging event that can be handled by the creator
    public class LogEventArgs : EventArgs 
    {
        private string log_text;

        public LogEventArgs(string log_text) 
        {
	...
	class WinspecInterfaceServer
	{
        ...
        private WINX32Lib.SpectroObjMgr spec;
        ...
        public void open(int port)
        {
            ...
            server.Port = port;
            server.Open();
            OnLog(new LogEventArgs(string.Format("Winspect Interface Server started.  Listening on port {0}", port)));
        }
        ...
        {
            OnLog(new LogEventArgs("Processing Command: " + full_command));
            ...
            switch (command_elements[0].ToLower())
            {
                case "acquire":
                    if (is_acquiring())
                        ...
                        {
                            OnLog(new LogEventArgs("Error starting acquisition: " + e.ToString()));
                ...
                case "status":
                    ...
                    {
                        OnLog(new LogEventArgs("Error getting status: " + e.ToString()));
                        send_response("err " + e.Message + "\n", connection);
                    }
                    break;
                ...
                case "set_acq_time":
                    ...
                    {
                        OnLog(new LogEventArgs("Error setting acquisition time\n"));
                        send_response("err " + e.Message + "\n", connection);
                    }
                    break;
                ...
                case "get_acq_time":
                    try
                    {
                        ...
                        OnLog(new LogEventArgs(string.Format("result = {0}", res)));
                        send_response(String.Format("ok {0}\n", acq_time_out), connection);
                    }
                    catch (Exception e)
                    {
                        OnLog(new LogEventArgs("Error fetching acquisition time\n"));
                        send_response("err " + e.Message + "\n", connection);
                    }
                    break;
                ...
                case "set_grating":
                    ...
                    {
                        OnLog(new LogEventArgs("Error setting grating\n"));
                        send_response("err " + e.Message + "\n", connection);
                    }
                    break;
                ...
                case "get_grating":
                    ...
                    {
                        OnLog(new LogEventArgs("Error fetching grating\n"));
                        send_response("err " + e.Message + "\n", connection);
                    }
                    break;
                ...
                case "reinitialize":
                    ...
                    {
                        OnLog(new LogEventArgs("Error reinitialzing winspec automation objects\n"));
                        send_response("err " + e.Message + "\n", connection);
                    }
                    break;
            }
        ...
        private void send_current_data(TcpServerConnection connection)
        {
            ...
            header.bin_x = (int)doc.GetParam(DM_CMD.DM_ROI_GP_X, out res);
            header.bin_y = (int)doc.GetParam(DM_CMD.DM_ROI_GP_Y, out res);
            ...
            for (int i_frame = 1; i_frame <= header.frame_count; i_frame++)
            {
                ...
                {
                    Console.WriteLine("Failed to write frame data to stream:  " + e.Message);
                }
            }
        }
        ...
        private void start_acquisition()
        {
            ...
            //short res;
            //object inuse = doc.GetParam(DM_CMD.DM_EXPERIMENTTIMEUTC, out res);
            //Console.WriteLine(string.Format("In Use Before: {0}", inuse));
            exp.Start(ref doc);

            //inuse = (int)doc.GetParam(DM_CMD.DM_EXPERIMENTTIMEUTC, out res);
            //Console.WriteLine(string.Format("In Use After:  {0}", inuse));
