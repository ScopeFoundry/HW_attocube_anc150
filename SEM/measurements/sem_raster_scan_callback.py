...
class ScanCheck(QtCore.QObject):
    ...
    def wait(self):
        while self.images.finished==False and self.parent.stop_pushButton.isDown()==False and self.parent.gui.ui.isVisible():
...
class SemRasterScanCallBack(Measurement):

    name = "sem_raster_scan_callback"
    ...
    def _run(self):
        ...
        '''
        Check to see if the scan area is set to square, if it is
        set the lines equal to points
        '''       
        ...
        '''
        read magnification and setup scale bar converter, also specify the pixel size
        '''
        ...
        '''
        create a ImageData object, which handles the image buffer stored inside the memory, it can be read out by
        the figures or be saved
        '''
        ...
        self.single_scan.update_value(1)        
        if self.single_scan.val==1:
            ...
            if self.scanner.auto_blanking.val:
                if hasattr(self,"sem_remcon"):
                    if self.sem_remcon.connected.val:
                        self.sem_remcon.remcon.write_beam_blanking(1)
    ...
    def save_current_view(self,fname):
            ...
            collection.save_measurement_component(self.dict_logged_quantity_val(self.logged_quantities), 
                                                  self.dict_logged_quantity_unit(self.logged_quantities))
            self.sem_remcon=self.gui.sem_remcon
            collection.save_hardware_component('sem_remcon', 
                                               self.dict_logged_quantity_val(self.sem_remcon.logged_quantities), 
                                               self.dict_logged_quantity_unit(self.sem_remcon.logged_quantities))
    ...
    def auto_save(self):
        fname=self.gui.folder.val+'/'+self.gui.filename.val+str(self.gui.filenumber.val)
        while os.path.isfile(fname+'.hdf5') or os.path.isfile(fname+'.txt'):
