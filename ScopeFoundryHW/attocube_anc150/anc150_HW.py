...
class ANC_HW(HardwareComponent):
    ...
    def setup(self):
        ...
        self.port = self.settings.New(name="port", initial="COM5", dtype=str, ro=False)
    ...
    def connect(self):
        self.anc_interface = ANC_Interface(port=self.port , debug=self.settings['debug_mode'])

        for i in [1,2,3,4,5,6]:
            self.settings['voltage'+str(i)].connect_to_hardware(
                read_func = self.read_active_axis_voltage,
                write_func = self.write_active_axis_voltage)
            self.settings['capacitance'+str(i)].connect_to_hardware(
                read_func = self.read_active_axis_cap)
            self.settings['frequency'+str(i)].connect_to_hardware(
                read_func = self.read_active_axis_freq,
                write_func = self.write_active_axis_freq)
            self.settings['axis_mode'+str(i)].connect_to_hardware(
